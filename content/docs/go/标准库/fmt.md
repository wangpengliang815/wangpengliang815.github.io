`fmt` 包实现了类似C#语言 `Console.Writeline` 和 `Console.ReadLine` 的格式化I/O。主要分为输出内容和获取输入两部分。
...
{wangpengliang}
{name:wangpengliang}
struct { name string }{name:"wangpengliang"}
struct { name string }
100%
```
...
wangpengliang
you name is：wangpengliang   
please input you name,age,sex
wangpengliang 18 true
扫描结果 name:wangpengliang age:18 sex:true
```
...
- Scanf从标准输入扫描文本，根据 `format` 参数指定的格式去读取由空白符分隔的值保存到传递给本函数的参数中
...
将上面的代码编译后在终端执行，在终端按照指定的格式依次输入`1:wpl 2:18 3:true`。

```bash
PS D:\GoProject\src\golang.basic> go run .\fmtTest.go
1:wpl 2:18 3:true
扫描结果 name:wpl age:18 sex:true 
```
...
将上面的代码编译后在终端执行，在终端依次输入`1`、`2`和`false`使用空格分隔。

```bash
PS D:\GoProject\src\golang.basic> go run .\fmtTest.go
1 2 false
扫描结果 name:1 age:2 sex:false
```
...
有时候需要完整获取输入的内容，而输入的内容可能包含空格，这种情况下可以使用 `bufio` 包来实现。比如：
